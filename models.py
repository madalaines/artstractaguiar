...
def cleanup_unlinked_files(artwork_folder):
    conn = get_db_connection()
    cursor = conn.cursor()

    cursor.execute("SELECT filename FROM Artwork")
    db_files = {row[0] for row in cursor.fetchall()}

    folder_files = {file for file in os.listdir(artwork_folder) if file.endswith('.png')}

    unlinked_files = folder_files - db_files

    for file in unlinked_files:
        os.remove(os.path.join(artwork_folder, file))
        print(f"Deleted unlinked file: {file}")

    conn.close()

def delete_rejected_artwork(artwork_id, artwork_folder):
    conn = get_db_connection()
    cursor = conn.cursor()

    # Get the file name from the database
    cursor.execute("SELECT filename FROM Artwork WHERE id = ?", (artwork_id,))
